...
function SecurityEditor() {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {/* Veritabanı & Sunucu Bağlantı Teşhisi */}
      <DiagnosticsCard />
...
function DiagnosticsCard() {
  ...
  const runTest = async () => {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        setDiag({ error: data.error || "Teşhis çalıştırılamadı." })
      }
    } catch (e: any) {
      setDiag({ error: e?.message || "Sunucuya ulaşılamadı." })
    } finally {
      setTesting(false)
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-[#e6ebf2] bg-white p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#f1f4f8] pb-4 mb-5">
        <div>
          <h3 className="text-base font-bold text-[#0f2747]">🛠️ Veritabanı ve Canlı Sistem Teşhisi</h3>
          <p className="text-xs text-[#71809b] mt-0.5">
            Canlıda (Vercel) içeriklerin kalıcı kaydedilmesini sağlayan Supabase veritabanı durumunu test edin.
          </p>
        </div>
        <Button
          onClick={runTest}
          disabled={testing}
          className="rounded-xl bg-[#0f2747] px-4 py-2 text-xs font-semibold text-white hover:bg-[#1a3860]"
        >
      ...
      {!tested && (
        <p className="text-xs text-[#8c99ae]">
          Canlı ortamda (cdabroad.com) veritabanınızın ve ortam değişkenlerinizin doğru çalışıp çalışmadığını anlamak için yukarıdaki butona tıklayın.
        </p>
      )}
          ...
          {diag.error ? (
            ...
            <>
              <div className="grid gap-3 sm:grid-cols-2">
                {/* 1. Supabase Değişkenleri */}
                <div className="rounded-xl border border-[#e8edf5] bg-[#fafbfd] p-3.5">
                  <div className="text-xs font-semibold text-[#50617d]">1. Vercel Değişkenleri</div>
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    {diag.supabase.configured ? (
                      <span className="flex items-center gap-1.5 font-bold text-[#18a058]">
                        <Check className="size-4" /> Tanımlı ({diag.supabase.url})
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5 font-bold text-amber-600">
                        <X className="size-4" /> Tanımlanmamış veya Eksik
                      </span>
                    )}
                  </div>
                </div>

                {/* 2. Supabase Tablo Durumu */}
                <div className="rounded-xl border border-[#e8edf5] bg-[#fafbfd] p-3.5">
                  <div className="text-xs font-semibold text-[#50617d]">2. Veritabanı Tablosu</div>
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    {diag.supabase.tableExists ? (
                      <span className="flex items-center gap-1.5 font-bold text-[#18a058]">
                        <Check className="size-4" /> website_content tablosu hazır
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5 font-bold text-red-600">
                        <X className="size-4" /> Tablo Bulunamadı
                      </span>
                    )}
                  </div>
                </div>
                ...
                {/* 3. Yazma Testi */}
                <div className="rounded-xl border border-[#e8edf5] bg-[#fafbfd] p-3.5">
                  <div className="text-xs font-semibold text-[#50617d]">3. Veri Kaydetme İzni</div>
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    {diag.supabase.writeTestPassed ? (
                      <span className="flex items-center gap-1.5 font-bold text-[#18a058]">
                        <Check className="size-4" /> Kaydetme Başarılı (Aktif)
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5 font-bold text-red-600">
                        <X className="size-4" /> Yazma Başarısız
                      </span>
                    )}
                  </div>
                </div>
                ...
                {/* 4. Dosya Sistemi */}
                <div className="rounded-xl border border-[#e8edf5] bg-[#fafbfd] p-3.5">
                  <div className="text-xs font-semibold text-[#50617d]">4. Çalışma Ortamı</div>
                  <div className="mt-2 text-xs font-medium text-[#50617d]">
                    {diag.filesystem.mode}
                  </div>
                </div>
              </div>

              {/* Hata ve Çözüm Rehberi */}
              {diag.supabase.errorMessage && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900">
                  <div className="font-bold mb-1">Tespit Edilen Sorun:</div>
                  <div className="font-mono bg-white/70 p-2 rounded-lg border border-amber-200 mb-2">
                    {diag.supabase.errorMessage}
                  </div>
                  <div className="text-[11px] leading-relaxed">
                    💡 <strong>Çözüm:</strong> Supabase panelinizde <strong>SQL Editor</strong> sekmesine gidip <code>website_content</code> tablosunu oluşturan SQL kodunu çalıştırmanız ve Vercel'deki <code>NEXT_PUBLIC_SUPABASE_URL</code> ve <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> değerlerini kontrol etmeniz önerilir.
                  </div>
                </div>
              )}

              {diag.supabase.writeTestPassed && (
                <div className="rounded-xl border border-[#a3e5c0] bg-[#e7f7ef] p-3 text-xs font-medium text-[#0f6b3b]">
                  ✓ Tüm veritabanı bağlantıları kusursuz çalışıyor! Panelden yaptığınız her değişiklik anında veritabanına kaydedilir ve ana sayfada görünür.
                </div>
              )}
            </>
          )}
